...
postprocess.py
--------------
Reads results.csv (1000 MC samples) and results_sparse.csv (83 quadrature nodes) and performs:
  1. MC statistics        — mean, std, CoV, convergence plots, histograms
  2. Regression PCE       — fit via least squares on 1000 MC samples, orders 2-5
  3. Quadrature PCE       — fit via cp.fit_quadrature on 83 sparse nodes, order 3
  4. Comparison           — regression PCE vs quadrature PCE vs MC histogram
  5. Sobol indices        — first-order and total sensitivity indices from both PCEs
...
plt.suptitle("PCE Surrogate Comparison: Regression vs Quadrature vs MC",
             fontsize=12, y=1.02)
plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, "pce_comparison.png"), dpi=150, bbox_inches="tight")
print(f"\nSaved pce_comparison.png")
plt.close()
...
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print("5. SOBOL SENSITIVITY INDICES")
print("=" * 60)
 
print("\n  --- From Regression PCE (order 2) ---")
reg_sobol_S1 = {}
reg_sobol_ST = {}
for qoi, label in QOIS.items():
    fitted = reg_models[qoi][BEST_REG_ORDER]
    S1 = cp.Sens_m(fitted, joint)
    ST = cp.Sens_t(fitted, joint)
    reg_sobol_S1[qoi] = S1
    reg_sobol_ST[qoi] = ST
    print(f"\n  QoI: {qoi}")
    for j, inp in enumerate(INPUT_LABELS):
        print(f"    {inp:15s}  S1={S1[j]:.4f}   ST={ST[j]:.4f}")
 
print("\n  --- From Quadrature PCE (order 3) ---")
quad_sobol_S1 = {}
quad_sobol_ST = {}
for qoi, label in QOIS.items():
    fitted = quad_models[qoi]
    S1 = cp.Sens_m(fitted, joint)
    ST = cp.Sens_t(fitted, joint)
    quad_sobol_S1[qoi] = S1
    quad_sobol_ST[qoi] = ST
    print(f"\n  QoI: {qoi}")
    for j, inp in enumerate(INPUT_LABELS):
        print(f"    {inp:15s}  S1={S1[j]:.4f}   ST={ST[j]:.4f}")
 
# ── Sobol bar charts — Regression PCE ─────────────────────────────────────────
fig, axes = plt.subplots(1, len(QOIS), figsize=(14, 5))
...
for col, (qoi, label) in enumerate(QOIS.items()):
    ...
    ST = reg_sobol_ST[qoi]
    ax = axes[col]
    ax.bar(x - width/2, S1, width, label="First-order $S_i$",  color="steelblue")
    ax.bar(x + width/2, ST, width, label="Total $S_{Ti}$",     color="darkorange", alpha=0.8)
    ax.set_title(f"Sobol Indices (Regression)\n{qoi}")
    ax.set_xticks(x)
...
plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, "sobol_regression.png"), dpi=150)
print(f"\nSaved sobol_regression.png")
plt.close()
 
# ── Sobol bar charts — Quadrature PCE ─────────────────────────────────────────
fig, axes = plt.subplots(1, len(QOIS), figsize=(14, 5))
for col, (qoi, label) in enumerate(QOIS.items()):
    S1 = quad_sobol_S1[qoi]
    ST = quad_sobol_ST[qoi]
    ax = axes[col]
    ax.bar(x - width/2, S1, width, label="First-order $S_i$",  color="steelblue")
    ax.bar(x + width/2, ST, width, label="Total $S_{Ti}$",     color="darkorange", alpha=0.8)
    ax.set_title(f"Sobol Indices (Quadrature)\n{qoi}")
    ax.set_xticks(x)
    ax.set_xticklabels(INPUT_LABELS, fontsize=11)
    ax.set_ylabel("Sensitivity index")
    ax.set_ylim(0, 1.1)
    ...
    ax.legend(fontsize=8)
plt.tight_layout()
plt.savefig(os.path.join(PLOT_DIR, "sobol_quadrature.png"), dpi=150)
print(f"Saved sobol_quadrature.png")
plt.close()
 
# ──────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 60)
print(f"  Monte Carlo (reference):          {N_mc} MOOSE calls")
print(f"  Regression PCE order {BEST_REG_ORDER}:          {N_mc} MOOSE calls (reuses MC)")
print(f"  Quadrature PCE order {QUAD_ORDER}:           {N_quad} MOOSE calls")
print(f"  Reduction factor:                  {N_mc/N_quad:.1f}x fewer calls for quadrature")
print("=" * 60)
print("Postprocessing complete.")
